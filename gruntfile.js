...
module.exports = function init(grunt) {
    ...
    grunt.initConfig({
        ...
        run: {
            ...
            copyArticlesToFirebase: { args: ['-m', 'cp', '-r', 'public/articles/*', `${firebaseStorageUrl}articles`], cmd: 'gsutil' },
            copyBuildKitsToFirebase: { args: ['-m', 'cp', '-r', 'public/buildKits/*', `${firebaseStorageUrl}buildKits`], cmd: 'gsutil' },
            copyEmulatorsToFirebase: { args: ['-m', 'cp', '-r', 'public/emulators/*', `${firebaseStorageUrl}emulators`], cmd: 'gsutil' },
            copyFileStoreToFirebase: { args: ['-m', 'cp', '-r', 'public/fileStore/*', `${firebaseStorageUrl}fileStore`], cmd: 'gsutil' },
            copyPresentationBooksToFirebase: { args: ['-m', 'cp', '-r', 'public/presentationBooks/*', `${firebaseStorageUrl}presentationBooks`], cmd: 'gsutil' },
